...
def get_args_parser():
    ...
    parser.add_argument('--demo_path', type=str, default='./crop2', help='Path to experiment folder')
    ...
    #Parser for Confidence Score
    parser.add_argument('--confidence_threshold', type=float, default=0.5, help='Confidence threshold for reference frame reset')

    return parser

#Confidence metric calculation
def calculate_confidence_metric(pred):
    conf = pred['conf']
    conf_sig = (conf - 1) / conf
    return conf_sig.mean()
...
def main(args):
    ...
    ##### Load model
    #model = Spann3R(dus3r_name='./checkpoints/DUSt3R_ViTLarge_BaseDecoder_512_dpt.pth', use_feat=False).to(device)
    model = Spann3R(dus3r_name='./checkpoints/DUSt3R_ViTLarge_BaseDecoder_512_dpt.pth', use_feat=False, confidence_threshold=args.confidence_threshold).to(device)
   
    #model.load_state_dict(torch.load(args.ckpt_path)['model'])
    model.load_state_dict(torch.load(args.ckpt_path, map_location=torch.device('cpu'))['model'])
    model.eval()
    ...
    preds, preds_all, sp_mem = model.forward(batch)

    # After processing, you can access all reference frames and transformations
    for i, ref_frame in enumerate(model.reference_frames):
        print(f"Reference frame {i}: Shape {ref_frame['shape']}")

    # Transform points to global coordinate system
    for i, pred in enumerate(preds):
        print(f"Available keys in pred: {pred.keys()}")
        
        if 'pts3d' in pred:
            points_3d = pred['pts3d']
        elif 'pts3d_in_other_view' in pred:
            points_3d = pred['pts3d_in_other_view']
        else:
            print(f"Warning: No 3D points found in prediction {i}")
            continue

        global_points = model.transform_to_global(points_3d, i)
