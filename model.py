...
class Spann3R(nn.Module):
    #Added the confidence threshold in the argument for frame reset.
    def __init__(self, dus3r_name="./checkpoints/DUSt3R_ViTLarge_BaseDecoder_512_dpt.pth", 
                 use_feat=False, mem_pos_enc=False, memory_dropout=0.15, confidence_threshold=0.5):
        ...
        #Initialization for frame reset.
        self.confidence_threshold = confidence_threshold
        self.reference_frames = []
        self.global_transform = []

    # Function for Spann3r frame update
    def calculate_confidence_metric(self, res1, res2):
        conf1 = res1['conf']
        conf2 = res2['conf']
        conf1_sig = (conf1 - 1) / conf1
        conf2_sig = (conf2 - 1) / conf2
        return (conf1_sig.mean() + conf2_sig.mean()) / 2

    def reset_reference_frame(self, feat, pos, shape):
        new_ref_frame = {
            'feat': feat.detach().clone(),
            'pos': pos.detach().clone(),
            'shape': shape.detach().clone(),
            'index': len(self.reference_frames)
        }
        self.reference_frames.append(new_ref_frame)
    
    # transformation from previous reference frame
        if len(self.reference_frames) > 1:
            prev_ref = self.reference_frames[-2]
            transform = self.calculate_transform(prev_ref, new_ref_frame)
            self.global_transform.append(transform)
        else:
            # For the first reference frame
            self.global_transform.append(torch.eye(4, device=feat.device))

    def calculate_transform(self, prev_ref, new_ref):
        # feature extraction from previous and new reference frames
        prev_feat = prev_ref['feat'].detach().cpu().numpy()
        new_feat = new_ref['feat'].detach().cpu().numpy()

        # Reshape features to 2D arrays (flattening all dimensions except the last)
        prev_pts = prev_feat.reshape(-1, prev_feat.shape[-1])
        new_pts = new_feat.reshape(-1, new_feat.shape[-1])

        # dimensions for the transformation
        if prev_pts.shape[1] < 3:
            prev_pts = np.pad(prev_pts, ((0, 0), (0, 3 - prev_pts.shape[1])))
            new_pts = np.pad(new_pts, ((0, 0), (0, 3 - new_pts.shape[1])))
        elif prev_pts.shape[1] > 3:
            prev_pts = prev_pts[:, :3]
            new_pts = new_pts[:, :3]
        ...
        new_centered = new_pts - new_centroid

        # the covariance matrix
        H = prev_centered.T @ new_centered
        ...
        # translation
        t = new_centroid - R @ prev_centroid
    ...
    def transform_to_global(self, points, frame_index):
        global_transform = torch.eye(4)
        for i in range(frame_index):
            if i < len(self.global_transform):
                global_transform = torch.matmul(global_transform, self.global_transform[i])
    
        # Reshape points to (batch_size, num_points, 3)
        if len(points.shape) == 4:
            batch_size, height, width, _ = points.shape
            points = points.view(batch_size, height * width, 3)
        ...
        global_points = torch.matmul(homogeneous_points, global_transform.T)
        ...
        if len(points.shape) == 4:
            global_points = global_points.view(batch_size, height, width, 3)
    ...
    # New Forward function for frame resetting
    def forward(self, frames, return_memory=False):
        sp_mem = SpatialMemory(self.norm_q, self.norm_k, self.norm_v, mem_dropout=self.mem_dropout)
        ...
        for i in range(len(frames)):
            ...
            # confidence and reset if necessary
            confidence_metric = self.calculate_confidence_metric(res1, res2)
            if confidence_metric < self.confidence_threshold:
                self.reset_reference_frame(feat2, pos2, shape2)
                sp_mem.init_mem()  # Reset spatial memory
